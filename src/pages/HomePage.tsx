...
const DEFAULT_HOBBIES: HobbyItem[] = [
  { title: "Coordina", description: "Local-first model version control and optional cloud collaboration for Rhino 8—commits, history, restoration, issues, screenshots and team coordination in one focused workflow.", url: COORDINA, label: "RHINO 8 · BIM VERSION CONTROL" },
...
const withDefaultHobbies = (items: HobbyItem[] = []) => [
  ...DEFAULT_HOBBIES,
  ...items.filter((item) => !DEFAULT_HOBBIES.some((featured) => featured.title.toLowerCase() === item.title.toLowerCase() || featured.url.toLowerCase() === item.url.toLowerCase())),
].filter((item) => item.title !== "Gym Records");
...
export function HomePage() {
  ...
  useEffect(() => {
    getDoc(doc(db, "siteConfig", "themes")).then((snap) => {
      ...
      savedTechnical.hobbies = withDefaultHobbies(savedTechnical.hobbies);
      savedApple.hobbies = withDefaultHobbies(savedApple.hobbies);
